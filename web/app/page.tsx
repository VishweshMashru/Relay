...
import { FeedPanel, EventLog } from "@/components/console";

// Landing page as an operations console. The first viewport is the product
// working — a feed, an event stream, the API call that caused it — because a
// camera intelligence company's homepage should be watching something.
// Permanently dark; monospace is the voice; copy states facts.
    ...
    <nav className="sticky top-0 z-40 bg-neutral-950/90 backdrop-blur border-b border-neutral-800">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-4 md:px-6 h-12">
        <Link href="/" className="font-mono font-medium tracking-tight text-neutral-100">
          streamo<span className="text-emerald-500">_</span>
        </Link>
        <div className="flex items-center gap-4 md:gap-5 font-mono text-xs">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="hidden md:inline text-neutral-500 hover:text-neutral-100">
              {l.label}
            </a>
          ))}
          <Link href="/download" className="hidden sm:inline text-neutral-500 hover:text-neutral-100">
            Download
          </Link>
          <Show when="signed-out">
            <SignInButton mode="modal">
              <button className="text-neutral-500 hover:text-neutral-100">Sign in</button>
            </SignInButton>
            <SignUpButton mode="modal">
              <button className="border border-neutral-100 text-neutral-100 px-3 py-1 hover:bg-neutral-100 hover:text-neutral-950">
                Get a key
              </button>
            </SignUpButton>
          </Show>
          <Show when="signed-in">
            <Link href="/dashboard" className="border border-neutral-100 text-neutral-100 px-3 py-1 hover:bg-neutral-100 hover:text-neutral-950">
              Dashboard
            </Link>
            <UserButton />
          </Show>
...
function Console() {
  return (
    <header className="border-b border-neutral-800">
      <div className="max-w-6xl mx-auto px-4 md:px-6 pt-10 md:pt-14 pb-10 flex flex-col gap-8">
        <div className="flex flex-col gap-5 max-w-3xl">
          <p className="font-mono text-[11px] tracking-widest text-neutral-500">
            STREAMO — CAMERA INTELLIGENCE · <span className="text-emerald-500">API OPERATIONAL</span>
          </p>
          <h1 className="font-mono font-medium text-2xl md:text-4xl tracking-tight leading-snug text-neutral-50">
            Any camera. Any network.
            <br />
            Live, stored, and understood — from one API.
          </h1>
          <p className="text-sm md:text-base text-neutral-400 leading-relaxed max-w-2xl">
            An agent on the LAN dials out; nothing is port-forwarded. Streams exist while
            watched, clips persist exactly as long as you say, and vision models — including
            AI agents over MCP — can look at any frame.
          </p>
          <div className="flex flex-wrap items-center gap-3 font-mono text-sm">
            <Show when="signed-out">
              <SignUpButton mode="modal">
                <button className="bg-emerald-500 text-neutral-950 px-5 py-2.5 font-medium hover:bg-emerald-400">
                  Get an API key →
                </button>
              </SignUpButton>
            </Show>
            <Show when="signed-in">
              <Link href="/dashboard" className="bg-emerald-500 text-neutral-950 px-5 py-2.5 font-medium hover:bg-emerald-400">
                Open dashboard →
              </Link>
            </Show>
            <Link href="/showcase" className="border border-neutral-700 px-5 py-2.5 hover:border-neutral-100">
              Live demo
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-x-8 gap-y-2 font-mono text-[11px] text-neutral-500 border-t border-neutral-800 pt-4">
          <span>HLS 5–15s</span>
  ...
  return (
    <div className="border border-neutral-800 bg-neutral-950">
      <div className="flex items-center justify-between px-3 py-2 border-b border-neutral-800 font-mono text-[11px] tracking-wide">
        <span className="text-neutral-400">API — first stream, unedited</span>
        <span className="text-neutral-600">2.4s</span>
      </div>
      <pre className="p-3 font-mono text-[11.5px] leading-5 overflow-x-auto text-neutral-400">
{`$ curl api.streamo.in/v1/sessions -X POST \\
  -H "Authorization: Bearer rk_live_••••" \\
  -d '{"camera_id":"730b91a9-…"}'
`}
        <span className="text-neutral-600">{`{ "id": "5c6a196f-…",`}</span>
        {"\n"}
        <span className="text-emerald-500">{`  "viewer_url": "https://…/video.m3u8",`}</span>
        {"\n"}
        <span className="text-neutral-600">{`  "status": "pending" }`}</span>
        {"\n"}
        <span className="text-neutral-600"># t+15s: manifest live, video playing</span>
      </pre>
    </div>
  );
}
...
function Capabilities() {
  ...
  return (
    <section id="capabilities" className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
      <SectionHead n="01" title="Capabilities" />
      <div className="flex flex-col">
        {rows.map((r) => (
          <div key={r.name} className="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-6 py-5 border-t border-neutral-800 first:border-t-0">
...
function Architecture() {
  return (
    <section id="architecture" className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 border border-neutral-800 p-5 overflow-x-auto bg-black/40">
...
function Stack() {
  ...
  return (
    <section className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
      ...
      <div className="flex flex-col">
        {layers.map((l) => (
          <div key={l.n} className="grid grid-cols-12 gap-4 md:gap-6 py-5 border-t border-neutral-800">
...
function Roadmap() {
  ...
  return (
    <section id="roadmap" className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
      ...
      <p className="text-sm text-neutral-400 max-w-2xl -mt-4 mb-10 leading-relaxed">
        ...
        <a className="underline underline-offset-2 hover:text-neutral-100" href="mailto:hello@streamo.in">
          hello@streamo.in
        </a>
        .
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
        {tracks.map((t) => (
          <div key={t.track} className="py-4 border-t border-neutral-800">
...
function Properties() {
  ...
  return (
    <section className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
        {props.map(([k, v]) => (
          <div key={k} className="py-4 border-t border-neutral-800">
...
function Pricing() {
  ...
  return (
    <section id="pricing" className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 border-t border-neutral-800">
        {tiers.map((t) => (
          <div key={t.name} className="py-6 md:px-6 first:md:pl-0 last:md:pr-0 md:border-l border-neutral-800 first:md:border-l-0 flex flex-col gap-4 border-b md:border-b-0">
            ...
            <div className="mt-auto pt-2">
              {t.name === "Enterprise" ? (
                <a href="mailto:hello@streamo.in" className="font-mono text-sm underline underline-offset-4 hover:text-emerald-500">
                  {t.cta} →
                </a>
              ) : (
                <Show when="signed-out">
                  <SignUpButton mode="modal">
                    <button className="font-mono text-sm underline underline-offset-4 hover:text-emerald-500">{t.cta} →</button>
                  </SignUpButton>
                </Show>
              )}
            </div>
...
function FAQ() {
  ...
  return (
    <section id="faq" className="max-w-6xl mx-auto w-full px-4 md:px-6 py-14">
...
function Footer() {
  return (
    <footer className="border-t border-neutral-800 mt-auto">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 font-mono text-xs text-neutral-500">
        <span>
          streamo<span className="text-emerald-500">_</span> camera intelligence
        </span>
        <div className="flex items-center gap-5">
          <a href="#capabilities" className="hover:text-neutral-100">Capabilities</a>
          <a href="#roadmap" className="hover:text-neutral-100">Roadmap</a>
          <Link href="/showcase" className="hover:text-neutral-100">Demo</Link>
          <Link href="/download" className="hover:text-neutral-100">Download</Link>
          <a href="https://github.com/VishweshMashru/Relay" className="hover:text-neutral-100">GitHub</a>
        </div>
      </div>
    </footer>
  );
